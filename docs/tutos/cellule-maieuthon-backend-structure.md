# 🎮 Cellule Colab : Backend Maïeuthon - Version STRUCTURE

**Version adaptée pour `PROMPT_EVALUATION_STRUCTURE` qui génère un format JSON détaillé**

**À ajouter APRÈS la cellule 7 (API FastAPI) et AVANT la cellule 8 (Lancement Serveur)**

---

## 📝 Code de la cellule (VERSION STRUCTURE)

```python
# =============================================================================
# 🎮 MAÏEUTHON - Backend : Évaluation et Message Final (VERSION STRUCTURE)
# =============================================================================
...
# ⚠️ PROMPT_EVALUATION FINAL (version structurée qui fonctionnait mieux - format JSON simple)
...
def evaluer_dialogue(dialogue: str, score_front: int) -> dict:
    ...
    # Augmenter max_new_tokens pour le format STRUCTURE qui peut être plus long
    with torch.autocast(device_type=device_type, dtype=dtype):
        outputs_eval = model.generate(
            **inputs_eval,
            max_new_tokens=100,  # Format simple, pas besoin de plus
...
print("✅ Endpoint /evaluate créé pour Maïeuthon (VERSION STRUCTURE)")
```
...
1. **`max_new_tokens` augmenté** : 500 au lieu de 100 (pour gérer le JSON structuré plus long)
2. **Pattern JSON amélioré** : Gère les JSON imbriqués (format STRUCTURE)
3. **Parsing double format** : Détecte et parse les deux formats (ancien et STRUCTURE)
4. **Message final optionnel** : Utilise `message_final` du JSON STRUCTURE si présent, sinon génère avec `PROMPT_MESSAGE_FINAL`
5. **Debug amélioré** : Affiche les signaux détectés si format STRUCTURE
...
1. Vérifier que le prompt `PROMPT_EVALUATION` vient de `Backend/PROMPT_EVALUATION_STRUCTURE.py`
2. Exécuter la cellule
3. Vérifier que `✅ Endpoint /evaluate créé pour Maïeuthon (VERSION STRUCTURE)` s'affiche
4. Tester avec le script de calibration

---

**Note :** Cette version est rétrocompatible avec l'ancien format JSON (format simple avec `comprehension`, `cooperation`, `progression` à la racine).
